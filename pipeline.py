...
from ai.prompts import (
    jd_analysis_prompt,
    generative_synthesis_prompt, # NEW master prompt
    cover_letter_prompt,
...
def postprocess_resume_text(text: str) -> str:
    ...
    t = text.replace("\r\n", "\n").replace("\u00A0", " ")
    t = re.sub(r'\*\*(.*?)\*\*', r'\1', t)
    t = re.sub(r'^\s*#+\s*(.*?)\s*$', r'\1', t, flags=re.MULTILINE) # Remove markdown headers
    t = re.sub(r'\n{3,}', '\n\n', t)
    return t.strip()
...
class ProResumePipeline:
    ...
    def run_all(self, jd, resume, keys: Dict) -> Dict:
        ...
        # 1) Analyze JD to get role and keywords
        ...
        # 2) Synthesize a new resume using the AI expert persona
        ...
        # 3) Post-analysis on the *newly generated* resume
        present_keywords, missing_keywords = [], []
        rlow = final_resume.lower()
        for k in jd_keywords:
            if re.search(r"\b" + re.escape(k.lower()) + r"\b", rlow):
        ...
        # 4) Generate Cover Letter using the new, superior resume
        cover_letter = self._text(cover_letter_prompt(jd_txt, final_resume, job_title), keys, order)

        # 5) Generate Interview Prep and Skills Gap
        interview_guide = self._text(interview_prep_prompt(jd_txt, final_resume), keys, order)
        skills_gap_report = self._json(skills_gap_prompt(jd_txt, rs_txt), keys, order) # Gap is vs original resume
